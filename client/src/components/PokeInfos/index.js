...
const PokeInfos = ({isSelected, setSelected}) => {
    ...
    const handleArrows = useCallback((e) => {
        if (e.key === "ArrowRight") {
    ...
    return (
        <div className={"pokeInfos " + (isSelected !== null ? "pokeInfosOpened" : "")}>
            ...
            <h3 className="pokeTypes">{pokemon?.types.map((type) => type.type.name.toUpperCase()).join(", ")}</h3>
            <img className="pokeSprite" src={pokemon?.sprites?.front_default} alt={pokemon?.name} />
